...
void main() {
  runApp(silhouetteEasy());
}

class silhouetteEasy extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Silhouette Easy',
      theme: ThemeData(
...
class SilhouetteGame extends StatefulWidget {
  @override
  _SilhouetteGameState createState() => _SilhouetteGameState();
}

class _SilhouetteGameState extends State<SilhouetteGame> {
  double _zoomLevel = 50.0;
  int _zoomOutCount = 0;
  final int _maxZoomOuts = 5;
  late String _imagePath;
  late String _correctAnswer;
  String _userGuess = '';
  int _points = 100;
  final int _pointDeduction = 10;
  ...
  void _onGuessSubmitted() {
    ...
    setState(() {
      ...
      if (isCorrect) {
        ...
        if (_zoomOutCount < _maxZoomOuts) {
          _zoomLevel = max(1.0, _zoomLevel - 3.0);
          _zoomOutCount++;
        }
        _points = max(10, _points - _pointDeduction);
        _controller.clear();
        _userGuess = '';
      }
    });
        ...
        child: Column(
          children: [
            ...
            Text.rich(
              TextSpan(
                text: 'Easy Mode: ',
                style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, fontStyle: FontStyle.italic, color: Colors.orange),
                children: [
                  TextSpan(
                    text: 'Image will zoom out after each attempt.',
                    style: TextStyle(fontSize: 12, fontWeight: FontWeight.normal, fontStyle: FontStyle.italic, color: Colors.orange),
                  ),
                ],
              ),

...
void main() {
  runApp(VanillaAdvanced());
}

class VanillaAdvanced extends StatelessWidget {
  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Vanilla: Advanced',
...
class Vanilla_adv extends StatefulWidget {
  @override
  advVanilla createState() => advVanilla();
}

class advVanilla extends State<Vanilla_adv> {
  ...
  Future<void> loadSelection() async {
    ...
    setState(() {
      selectedTheme = prefs.getString('selectedTheme') ?? 'No theme selected';
      selectedAnimal = prefs.getString('selectedAnimal') ?? 'No animal selected';
    });
  }
    ...
    if (guessedAnimal.isNotEmpty) {
      setState(() {
        attempts.insert(0, {
          ...
          'colors': {
            'diet': guessedAnimal['diet'] == animals.firstWhere((animal) => animal['name'].toLowerCase() == selectedAnimal.toLowerCase())['diet'] ? Colors.green : Colors.red,
          ...
          if (remainingGuesses == 0) {
            ...
            showDialog(
              ...
              builder: (context) => AlertDialog(
                ...
                content: Text('You have used all your guesses! The correct answer was $selectedAnimal.'),
                actions: [
                  TextButton(
                    onPressed: () => Navigator.of(context).pop(),
                    child: Text('OK'),
